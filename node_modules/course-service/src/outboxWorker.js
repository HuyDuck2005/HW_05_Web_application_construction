...
const POLL_INTERVAL_MS = Number(process.env.COURSE_OUTBOX_POLL_INTERVAL_MS ?? 2000);
...
async function processRow(row) {
  ...
  await publishCourseEvent({ routingKey: row.routing_key, event });
  await markOutboxEventPublished(row.id);
  console.log(`[course-outbox-worker] published eventId=${row.id} type=${row.event_type}`);
}
  ...
  while (!shouldStop) {
    ...
    for (const row of rows) {
      try {
        ...
        console.error(`[course-outbox-worker] failed eventId=${row.id}:`, error.message);
